...
-- 2. Grant access to public.users for Academicians in the same institution
...
-- 3. Grant access to student_profiles for Academicians in the same institution
...
USING (
  ...
  EXISTS (
    ...
    AND u.institution_id = public.get_my_institution_id()
  )
);

-- 4. Grant access to applications for Academicians in the same institution
...
-- 5. Grant access to application_skill_feedback for Academicians in the same institution
